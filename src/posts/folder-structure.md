...
프로젝트 폴더 구조에 대해 진지하게 고민해본 적이 있나요? 이 글은 웹 개발을 주제로 합니다. 굳이 구분하자면 백엔드에 가깝지만, 독립적인 프로젝트로 개발하는 현대 프론트엔드라면 이야기는 크게 다르지 않아요.

개발자들은 코드의 품질을 높이기 위해 객체지향 설계 기법과 디자인 패턴을 고민합니다. 그런데 의외로 폴더 구조에 대해서는 좋은 코드를 작성하는 방법만큼 깊은 논의가 없어요. 코드가 전자 부품이라면, 프로젝트 폴더 구조는 회로 설계입니다. 부품이 아무리 정밀해도 회로가 뒤엉켜 있으면 전류는 엉뚱한 곳으로 흘러요. 코드도 마찬가지예요. 개별 코드의 품질이 아무리 좋더라도, 폴더 구조와 참조 방향이 뒤엉켜 있으면 프로젝트 전체가 길을 잃습니다.
...
레이어드 아키텍처 자체가 이런 물리적 구조를 강제하는 건 아닙니다. 하지만 계층 간 단방향 흐름이라는 명확한 원칙이 있기 때문에 느낌대로 구성해도 처음에는 문제가 없어요. 구성이 쉽고, 코드의 큰 흐름이 눈에 보입니다.
...
도메인에 관련된 코드들이 여러 폴더에 파편화됩니다. 유저 관련 코드를 수정하려면 `controller/`, `service/`, `repository/`, `dto/`를 전부 돌아다녀야 해요. 이건 이 구조에서 어찌 보면 당연한 단점입니다.
...
DTO에서는 이 문제가 더 두드러집니다. DTO는 `controller`, `service`, `repository` 어디서든 가져다 쓸 수 있는 공용 폴더의 성격을 가지면서도, 본질적으로는 특정 도메인에 응집되어야 합니다. 가장 많이 생성되는 파일 유형임에도 불구하고, 유형 기준으로만 묶어두면 도메인별 하위 분류 없이는 파일을 찾는 것조차 힘들어져요. 게다가 요청에서 들어온 DTO가 흐름의 끝까지 그대로 쓰인다는 보장도 없습니다. 각 레이어에서 가공한 DTO가 생길 수 있고, 그걸 나누는 기준은 점점 모호해집니다.

## 기능 기준으로 나누기

계층이 아니라 기능(feature) 단위로 폴더를 나누는 접근도 있습니다. NestJS를 접해보면, 프레임워크가 이 방향을 권장하고 있다는 걸 알 수 있어요. `user/`, `post/`, `auth/` 처럼 각 기능 폴더가 자신에게 필요한 controller, service, repository, dto를 모두 품고 있는 구조입니다.

도메인 응집도는 확실히 높아집니다. 유저 관련 코드를 수정할 때 `user/` 하나만 보면 되니까요. 하지만 이 방식은 기능들이 서로 협력하는 상황에서는 불리합니다.

각 기능이 프로세스 안에서 독립적으로 돌아가면 좋겠지만, 현실은 다릅니다. `UserService`가 `PostService`를 참조하거나, 한 도메인의 모델이 다른 도메인의 모델을 참조하는 일은 흔해요. 어떤 서비스가 다른 기능의 repository를 직접 참조하는 경우도 봅니다. 이게 괜찮은 건지 찜찜하면서도, 스스로 규칙을 만들어 일단 허용하게 됩니다.
...
계층 기준 구조는 적어도 코드의 큰 흐름을 제어하는 느낌을 줬습니다. 기능 기준 구조는 응집도를 얻는 대신, 그 큰 틀에서의 방향성을 잃어요.

## 클린 아키텍처는 답인가?

그럼 클린 아키텍처를 쓰면 되지 않겠냐고 물을 수 있습니다. 과연 클린 아키텍처가 이 문제에 명확한 답을 줄까요?
...
그래서인지 같은 클린 아키텍처를 표방하면서도 프로젝트마다 폴더 구조가 제각각이에요. 해석이 다양한 것 자체가 나쁜 건 아니에요. 다만 한 가지 점검해볼 건 있습니다 — 그 해석으로 만들어진 폴더 구조가 일관된 규칙을 지키고 있는지요.

코드의 의존 방향만큼 중요한 것이 폴더의 의존 방향입니다. 폴더 간에 양방향 참조가 존재하거나, 같은 레벨의 폴더가 서로를 참조하고 있다면 — 그 이유를 명확하게 설명할 수 있어야 해요.
...
저는 이 고민을 하다가 [FSD(Feature-Sliced Design)](https://feature-sliced.design/kr/docs/get-started/overview)를 만났어요. FSD는 프론트엔드 아키텍처 방법론입니다.

![FSD 구조 개요](https://feature-sliced.design/kr/assets/images/visual_schema-e826067f573946613dcdc76e3f585082.jpg)

FSD는 폴더를 세 가지 단위로 구분합니다.
...
솔직히 이 글도 하나의 해석입니다. 더 나은 구조를 만들 수 있다고 생각한다면, 그게 틀린 말은 아니에요.
...
폴더 구조에 정답은 없습니다. 하지만 **규칙은 있어야 합니다.** 그리고 그 규칙을 다른 사람에게 설명할 수 있어야 해요.

계층 기준이든 기능 기준이든 — 어떤 방식을 선택하더라도 중요한 건 참조 규칙이 있느냐, 그리고 예외를 만들 때 그 이유를 설명할 수 있느냐입니다.
